...
class ControllerThesis:
    ...
    @classmethod
    def createProjectThesis(cls, db, title, abstract, pdf_link):
        try:
            person_id = current_user.person_id
            session = db.session()
            sql = text(
                "CALL InsertThesisProject(:title, :abstract, :pdf_link, :person_id)"
            )
            sql = text(
                "INSERT INTO THESIS (title, abstract, submission_date, expiration_date, last_update_date, pdf_link, thesis_status_id, is_project) "
                "VALUES (:title, :abstract, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 2 YEAR), CURDATE(),:pdf_link, 1, 1); "
